    ...
    <div className="bg-white dark:bg-zinc-800 rounded-lg shadow-md p-6 border border-zinc-200 dark:border-zinc-700">
      <div className="flex items-start justify-between">
        <div>
          ...
          {trend && (
            <p className={`text-xs mt-2 font-semibold ${trend.startsWith("+") ? "text-green-600 dark:text-green-400" : trend.startsWith("-") ? "text-red-600 dark:text-red-400" : "text-zinc-600 dark:text-zinc-400"}`}>
              {trend}
            </p>
          )}
        </div>
